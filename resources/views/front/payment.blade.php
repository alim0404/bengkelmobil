    ...
    <main
        ...
        <div id="Top-nav" class="flex items-center justify-between px-4 pt-5">
            ...
            <div class="flex flex-col w-fit text-center">
                <h1 class="font-semibold text-lg leading-[27px]">Lakukan Pembayaran</h1>
                <p class="text-sm leading-[21px] text-[#909DBF]">Rawat Mobil Anda dengan Baik</p>
            </div>
            <div class="w-10 h-10 flex shrink-0">
            </div>
        </div>
        <div id="Order-details" class="flex flex-col gap-2 px-4 mt-[30px]">
            <h2 class="font-semibold">Detail Pemesanan</h2>
                    ...
                    <div class="flex items-center w-full gap-[10px] bg-white">
                        ...
                        <div class="flex flex-col">
                            <div class="flex items-center gap-1">
                                <p class="font-semibold w-fit">{{$Bengkel->nama}}</p>
                    ...
                    <div class="flex items-center w-full gap-[10px] bg-white justify-between">
                        <div class="flex items-center gap-[10px]">
                            <div class="w-[60px] h-[60px] flex shrink-0">
                                <img src="{{storage::url($ServisMobil->icon)}}" alt="icon">
                            </div>
                            <div class="flex flex-col h-fit">
                                <p class="font-semibold">{{ $ServisMobil->nama }}</p>
                                @if(isset($selectedVariant) && $selectedVariant)
                                <p class="text-xs text-[#5B86EF] font-medium">{{ $selectedVariant->nama }}</p>
                                @endif
                ...
                <div id="Price-details" class="flex flex-col gap-[10px]">
                    <div class="flex items-center justify-between">
                        <p class="text-sm leading-[21px]">
                            {{ $ServisMobil->nama }}
                            @if(isset($selectedVariant) && $selectedVariant)
                            <span class="text-xs text-[#5B86EF]">({{ $selectedVariant->nama }})</span>
                            @endif
                        </p>
                        <p class="font-semibold">
                            Rp
                            {{number_format(isset($selectedVariant) && $selectedVariant ? $selectedVariant->harga : $ServisMobil->harga, 0, ',', '.')}}
                        </p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="text-sm leading-[21px]">Biaya Pemesanan</p>
                        <p class="font-semibold">Rp {{number_format($bookingFee, 0, ',', '.')}}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="text-sm leading-[21px]">Total Keseluruhan</p>
                        <p class="font-bold text-xl leading-[30px] text-[#FF8E62]">
        ...
        <div class="flex h-full flex-1 mt-5">
            <form action="{{ route('front.booking.payment.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div id="Payment-info" class="flex flex-col gap-4">
                    <h2 class="font-semibold">Lakukan Pembayaran</h2>
                    <div class="flex items-center w-full gap-[10px] bg-white">
                        ...
                        <div class="flex flex-col">
                            <div class="flex items-center gap-1">
                                <p class="font-semibold w-fit">Auto Berkah</p>
                                <div class="w-[18px] h-[18px] flex shrink-0">
                                    <img src="{{asset('assets/images/icons/verify.svg')}}" alt="verified">
                                </div>
                            </div>
                ...
                <div class="flex flex-col gap-2">
                    <label for="Proof" class="font-semibold">Bukti Pembayaran</label>
                    <div
                ...
                <hr class="border-[#E9E8ED]">
                <button type="submit"
                    class="w-full rounded-full p-[12px_20px] bg-[#FF8E62] font-bold text-white">Konfirmasi
                    Pembayaran</button>
            </form>
