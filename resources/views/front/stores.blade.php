...
<main
    ...
    <div id="Top-nav" class="flex items-center justify-between px-4 pt-5">
        ...
        <div class="flex flex-col w-fit text-center">
            <h1 class="font-semibold text-lg leading-[27px]">
                {{$ServisMobil->nama }}
            </h1>
            <p class="text-sm leading-[21px] text-[#909DBF]">Kota {{$cityName}}, {{$bengkel->count()}} Bengkel</p>
        </div>
    ...
    <section id="Store-list" class="flex flex-col gap-6 px-4 mt-[30px]">
        ...
        <a href="{{route('front.details', $store->slug)}}" class="card">
            ...
            <div
                ...
                <div class="w-full h-[120px] flex shrink-0 overflow-hidden relative">
                    <img src="{{ Storage::url($store->gambar_pratinjau) }}" class="w-full h-full object-cover"
                        alt="thumbnail">
                    @if($store->status_operasional)
                    <p
        ...
        @empty
        <p>Belum ada bengkel penyedia layanan tersebut</p>
        @endforelse
    </section>
</main>
@endsection